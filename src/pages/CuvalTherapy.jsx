import React, { useState, useRef, useEffect } from 'react';

const MILESTONES = [10, 25, 50, 80, 100];

function CuvalTherapy() {
  const [rubCount, setRubCount] = useState(0);
  const [bestCount, setBestCount] = useState(0);
  const [isRubbing, setIsRubbing] = useState(false);
  const [milestoneFlash, setMilestoneFlash] = useState(null);
  const rubTimeoutRef = useRef(null);
  const flashTimeoutRef = useRef(null);

  useEffect(() => {
    return () => {
      clearTimeout(rubTimeoutRef.current);
      clearTimeout(flashTimeoutRef.current);
    };
  }, []);

  const handleMassage = () => {
    setRubCount((prev) => {
      const next = prev + 1;

      setBestCount((best) => Math.max(best, next));

      if (MILESTONES.includes(next)) {
        setMilestoneFlash(next);
        clearTimeout(flashTimeoutRef.current);
        flashTimeoutRef.current = setTimeout(() => setMilestoneFlash(null), 1200);
      }

      return next;
    });

    setIsRubbing(true);
    clearTimeout(rubTimeoutRef.current);
    rubTimeoutRef.current = setTimeout(() => setIsRubbing(false), 150);
  };

  const handleKeyDown = (e) => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault();
      handleMassage();
    }
  };

  const getCatReaction = () => {
    if (rubCount === 0) return 'Çuval seni bekliyor. Sakince karnına dokun...';
    if (rubCount > 0 && rubCount < 10) return 'Gözlerini hafifçe açtı. Devam et...';
    if (rubCount >= 10 && rubCount < 25) return 'Mırrr... Doğru frekansı yakaladın!';
    if (rubCount >= 25 && rubCount < 50) return 'Evet, tam orası! Gözleri tamamen kapandı...';
    if (rubCount >= 50 && rubCount < 80) return 'Gerinmeye başladı. Çuvalizm felsefesi damarlarına işliyor.';
    if (rubCount >= 80 && rubCount < 100) return "Çuval şu an Nirvana'ya giden yolda. Boyut değiştiriyor!";
    if (rubCount >= 100) return '🐾 TEBRİKLER! ÇUVAL\'IN EN SEVDİĞİ İNSAN OLDUN! 🐾';
  };

  const scaleRatio = Math.min(1 + rubCount * 0.0015, 1.15);

  // Görsel efektler 100 masajdan sonra artık büyümüyor — yoksa çok agresif
  // tıklayan ziyaretçilerde renk/gölge aşırı doyuma ulaşıp bozuk görünüyordu.
  const effectiveCount = Math.min(rubCount, 100);
  const saturateValue = 1 + effectiveCount * 0.01;
  const shadowBlur = effectiveCount;

  return (
    <div className="press-editorial-wrapper animate-fade" lang="tr" style={{ minHeight: '80vh' }}>
      <style>
        {`
          @keyframes purrShake {
            0% { transform: translate(0, 0) rotate(0deg); }
            25% { transform: translate(1.5px, 1.5px) rotate(0.5deg); }
            50% { transform: translate(0, 0) rotate(0deg); }
            75% { transform: translate(-1.5px, 1.5px) rotate(-0.5deg); }
            100% { transform: translate(0, 0) rotate(0deg); }
          }
          .purr-active img {
            animation: purrShake 0.15s linear infinite;
          }

          @keyframes milestonePop {
            0% { transform: translateY(6px) scale(0.9); opacity: 0; }
            30% { transform: translateY(0) scale(1.05); opacity: 1; }
            100% { transform: translateY(-10px) scale(1); opacity: 0; }
          }
          .milestone-flash {
            animation: milestonePop 1.2s ease-out forwards;
          }
        `}
      </style>

      <div className="container">
        <div className="section-header-editorial" style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <span className="archive-badge" style={{ display: 'inline-block', marginBottom: '0.5rem' }}>// STRES ATMA MERKEZİ</span>
          <h1 className="editorial-title" style={{ textTransform: 'none', margin: '0.5rem 0' }}>ÇUVAL TERAPİSİ</h1>
          <p className="editorial-subtitle">Günün yorgunluğunu atmak için Çuvalizm felsefesine katıl. Karnına tıkla ve masaj yap!</p>
        </div>

        <div className="game-container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '2rem 1rem' }}>
          <div style={{ display: 'flex', gap: '2.5rem', marginBottom: '3rem', alignItems: 'flex-end' }}>
            <div>
              <h2
                style={{
                  fontFamily: 'var(--font-heading)',
                  fontSize: '5rem',
                  color: rubCount >= 100 ? 'var(--accent-dark)' : 'inherit',
                  margin: 0,
                  transition: 'color 0.5s',
                  textShadow: rubCount >= 100 ? '0 0 20px rgba(84, 107, 65, 0.4)' : 'none',
                  position: 'relative',
                }}
              >
                {rubCount}
                {milestoneFlash && (
                  <span
                    className="milestone-flash"
                    style={{
                      position: 'absolute',
                      top: '-1.2rem',
                      left: '50%',
                      transform: 'translateX(-50%)',
                      fontSize: '1rem',
                      whiteSpace: 'nowrap',
                      color: 'var(--accent-dark)',
                    }}
                  >
                    ✨ yeni evre
                  </span>
                )}
              </h2>
              <span style={{ fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '3px', opacity: 0.5, display: 'block', marginTop: '-0.5rem' }}>
                MASAJ SKORU
              </span>
            </div>

            {bestCount > 0 && (
              <div>
                <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.6rem', color: 'var(--accent-dark)', margin: 0, opacity: 0.7 }}>
                  {bestCount}
                </h3>
                <span style={{ fontSize: '0.7rem', fontWeight: 'bold', letterSpacing: '2px', opacity: 0.45, display: 'block' }}>
                  REKOR
                </span>
              </div>
            )}
          </div>

          <div
            onClick={handleMassage}
            onKeyDown={handleKeyDown}
            role="button"
            tabIndex={0}
            aria-label="Çuval'a masaj yap"
            className={isRubbing ? 'purr-active' : ''}
            style={{
              width: 'min(300px, 70vw)',
              aspectRatio: '1 / 1',
              margin: '0 auto 3rem auto',
              cursor: 'pointer',
              backgroundColor: rubCount >= 100 ? 'rgba(255, 215, 0, 0.1)' : 'rgba(84, 107, 65, 0.05)',
              borderRadius: '50%',
              overflow: 'hidden',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              boxShadow:
                rubCount >= 100
                  ? '0 0 60px rgba(255, 215, 0, 0.5), inset 0 0 20px rgba(255, 215, 0, 0.3)'
                  : rubCount > 0
                  ? `0 0 ${shadowBlur}px rgba(84, 107, 65, 0.2)`
                  : 'none',
              transition: 'box-shadow 0.5s ease, background-color 0.5s ease, transform 0.1s ease',
              transform: `scale(${scaleRatio})`,
              userSelect: 'none',
              WebkitTapHighlightColor: 'transparent',
              touchAction: 'manipulation',
              outline: 'none',
            }}
          >
            <img
              src="/cuval.png"
              alt="Çuval"
              draggable="false"
              style={{
                width: '100%',
                height: '100%',
                objectFit: 'cover',
                filter: rubCount >= 100 ? 'saturate(2) contrast(1.2) drop-shadow(0 0 10px gold)' : `saturate(${saturateValue}) contrast(1.1)`,
                transition: 'filter 0.5s ease',
                pointerEvents: 'none',
              }}
            />
          </div>

          <p
            aria-live="polite"
            style={{
              fontFamily: 'var(--font-heading)',
              fontSize: rubCount >= 100 ? '1.5rem' : '1.2rem',
              color: rubCount >= 100 ? 'var(--accent-dark)' : 'inherit',
              minHeight: '40px',
              fontStyle: 'italic',
              transition: 'all 0.3s',
              fontWeight: rubCount >= 100 ? 'bold' : 'normal',
              maxWidth: '600px',
              margin: '0 auto',
              lineHeight: '1.4',
            }}
          >
            {getCatReaction()}
          </p>

          {rubCount > 0 && (
            <button
              className="editorial-link-btn-anchor reset-btn"
              onClick={() => setRubCount(0)}
              style={{ marginTop: '3rem', fontSize: '0.8rem', opacity: 0.8, padding: '0.5rem 1rem', background: 'transparent', border: '1px dashed currentColor' }}
            >
              ÇUVAL'I UYANDIR ⟲
            </button>
          )}
        </div>
      </div>
    </div>
  );
}

export default CuvalTherapy;
